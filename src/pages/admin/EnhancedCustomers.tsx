...
import { Users, Search, Eye, Phone, Mail, Calendar, PoundSterling, MessageSquare, Tag, Filter, TrendingUp, Plus } from 'lucide-react';
...
export default function EnhancedCustomers() {
  ...
  const handleCreateCustomerSubmit = async (e: React.FormEvent) => {
    ...
    if (!createCustomerForm.email || !createCustomerForm.full_name) {
      toast({
    ...
    try {
      ...
      const { data: existingCustomers, error: checkError } = await supabase
        .from('customers')
        .select('email')
        .eq('email', createCustomerForm.email.toLowerCase().trim());
      ...
      const { error: insertError } = await supabase
        .from('customers')
        .insert({
          email: createCustomerForm.email.toLowerCase().trim(),
          full_name: createCustomerForm.full_name.trim(),
      ...
      toast({
        ...
        description: `Customer ${createCustomerForm.full_name} has been created successfully.`,
      });
    ...
    <AdminLayout>
      <div className="space-y-6 p-4 md:p-6">
        ...
        <Dialog
          ...
          onOpenChange={(open) => { if (!open) setIsCreatingCustomer(false); }}
        >
            ...
            <form onSubmit={handleCreateCustomerSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  value={createCustomerForm.full_name}

...
-- Remove non-combined bookings (if any exist)
DELETE FROM public.bookings WHERE time_slot_id IN (
  SELECT id FROM public.time_slots WHERE service_type IN ('ice_bath', 'sauna')
);
